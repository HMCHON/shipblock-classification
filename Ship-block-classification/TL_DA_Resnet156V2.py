...
finetune_model = multi_gpu_model(finetune_model, gpus=gpu_num)
...
# filepath = "./checkpoint/" + "Resnet156V2" + "_model_weights.hdf5"
...
skeras.save_history_history('TL_DA_Resnet-152v2', history.history, fold = foldname)

score = finetune_model.evaluate(validation_generator, verbose = 0)
finetune_model.save(os.path.join(foldname, 'TL_DA_Resnet-152v2.hdf5'))
print('Output results are saved in', foldname)

hist_df = pd.DataFrame(history.history)

hist_json_file = 'TL_DA_Resnet-152v2.json' 
with open(hist_json_file, mode='w') as f:
    hist_df.to_json(f)

# or save to csv: 
hist_csv_file = 'TL_DA_Resnet-152v2.csv'
...
def plot_training(history):
    ...
    fig1 = plt.gcf()
    plt.show()
    fig1.savefig('TL_DA_Resnet_Training and Validation accuracy.png')
    plt.close(fig1)
    ...
    fig2 = plt.gcf()
    plt.show()
    fig2.savefig('TL_DA_Resnet_Training and validation loss.png')
